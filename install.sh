#!/usr/bin/env bash
# =============================================================================
# NetAgent Platform — Instalador completo (fresh server)
# Alvo: Debian 12 (Bookworm) · Raiz do projeto: repositório onde este script vive
#
# Orquestra:
#   Docker: Traefik 3, Postgres 16 + pgvector, Redis 7, Evolution API v2,
...
#
# Chaves: Postgres/Redis/JWT/Encryption/InternalAPI/EvolutionGlobal são geradas.
# Somente OPENAI_KEY é solicitada ao operador.
# =============================================================================
set -Eeuo pipefail
...
DEFAULT_DOMAIN_EVOLUTION="agenteevo.forumtelecom.com.br"
...
ORIG_DOMAIN_EVOLUTION="agenteevo.forumtelecom.com.br"
...
setup_firewall() {
  ...
  # Containers falam com a API/Agent no host via "host-gateway" (172.17.0.1).
  # Sem essas duas regras a UFW bloqueia o tráfego vindo da bridge docker0 e
  # o nginx-frontend retorna 504 ao proxy-passar /api e /socket.io.
...
EOF
  ...
  log ".env gerado com chaves novas (OpenAI preservada, Evolution regenerada)."
}

# ── 8. Substituir domínios e GID do grupo docker no compose ─────────────────
patch_compose() {
  step "Patch docker-compose.yml (domínios + GID docker)"
  local compose="${PROJECT_DIR}/docker-compose.yml"
  # 8a) Domínios e e-mail ACME
  sed -i \
    -e "s|${ORIG_DOMAIN_PLATFORM}|${DOMAIN_PLATFORM}|g" \
    -e "s|${ORIG_DOMAIN_EVOLUTION}|${DOMAIN_EVOLUTION}|g" \
  ...
  # Reescreve a lista de group_add do bloco traefik (próximas 2 linhas após 'group_add:')
  python3 - "$compose" "$docker_gid" <<'PY'
import sys, re
path, gid = sys.argv[1], sys.argv[2]
src = open(path).read()
# Substitui todas as ocorrências de '- "NNN"' (3 dígitos) dentro de group_add por - "<gid>".
# Restringe-se à primeira linha seguinte a 'group_add:' para não afetar outros blocos.
def repl(m):
    return m.group(1) + f'- "{gid}"'
src = re.sub(r'(group_add:\s*\n\s*)-\s*"\d+"', repl, src)
open(path, "w").write(src)
PY
  log "docker-compose.yml patcheado (domínios + docker GID=${docker_gid})."
}
...
start_containers() {
  ...
  # exporta vars do .env para o compose resolver ${POSTGRES_PASSWORD} etc.
...
# ── 13. Banco: cria DB evolution + aplica schema netagent ───────────────────
init_databases() {
  step "Inicializando bancos (netagent + evolution)"
  # Cria DB da Evolution (v2 requer banco próprio)
  docker exec -i netagent-postgres psql -U netagent -d netagent -tAc \
    "SELECT 1 FROM pg_database WHERE datname='evolution'" | grep -q 1 \
    || docker exec -i netagent-postgres psql -U netagent -d netagent -c \
       "CREATE DATABASE evolution OWNER netagent" >/dev/null
  log "DB 'evolution' OK."
...
print_summary() {
  ...
  echo -e "${BOLD}${GREEN}"
  echo "╔══════════════════════════════════════════════════════════════╗"
  echo "║   ✅  NetAgent Platform instalado com sucesso                ║"
  echo "╚══════════════════════════════════════════════════════════════╝"
  echo -e "${NC}"
  ...
  sep
  warn "Aponte DNS A/AAAA de ${DOMAIN_PLATFORM} e ${DOMAIN_EVOLUTION} para este servidor."
